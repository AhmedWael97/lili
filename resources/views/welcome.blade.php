...
<body class="font-sans antialiased text-gray-900 bg-white" x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 1000)">
    ...
    <!-- Pricing -->
    <div class="bg-gray-900 py-24" id="pricing">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-extrabold text-white sm:text-4xl">Hire Virtual Employees at Any Scale</h2>
                <p class="mt-4 text-xl text-gray-400">From solopreneurs to agencies - build your virtual workforce that fits your budget.</p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                @php
                    ...
                    $isPopular = strtolower($package->name) === 'professional';
                    $isFree = $package->price == 0;
                    $isAgency = strtolower($package->name) === 'agency';
                @endphp
                
                <div class="{{ $isPopular ? 'bg-white border-4 border-lili-500 transform scale-105' : 'bg-gray-800 border border-gray-700' }} rounded-2xl p-8 relative">
                    @if($isPopular)
                        <div class="absolute top-0 right-0 -mt-4 -mr-4 bg-lili-500 text-white text-xs font-bold px-3 py-1 rounded-full">Popular</div>
                    @endif
                    
                    <h3 class="text-xl font-semibold {{ $isPopular ? 'text-gray-900' : 'text-white' }}">{{ $package->name }}</h3>
                    <p class="mt-4 flex items-baseline {{ $isPopular ? 'text-gray-900' : 'text-white' }}">
                        <span class="text-5xl font-extrabold">${{ number_format($package->price, 0) }}</span>
                        <span class="ml-1 text-xl {{ $isPopular ? 'text-gray-500' : 'text-gray-400' }}">/mo</span>
                    </p>
                    <ul class="mt-6 space-y-4">
                        <li class="flex items-start {{ $isPopular ? 'text-gray-600' : 'text-gray-300' }}">
                            <span class="{{ $isPopular ? 'text-green-500' : 'text-green-400' }} mr-2">✓</span> 
                            {{ $features['facebook_pages'] == -1 ? 'Unlimited' : $features['facebook_pages'] }} Facebook {{ $features['facebook_pages'] == 1 ? 'Page' : 'Pages' }}
                        </li>
                        <li class="flex items-start {{ $isPopular ? 'text-gray-600' : 'text-gray-300' }}">
                            <span class="{{ $isPopular ? 'text-green-500' : 'text-green-400' }} mr-2">✓</span> 
                            {{ $features['posts_per_month'] == -1 ? 'Unlimited' : $features['posts_per_month'] }} posts/mo
                        </li>
                        <li class="flex items-start {{ $isPopular ? 'text-gray-600' : 'text-gray-300' }}">
                            <span class="{{ $isPopular ? 'text-green-500' : 'text-green-400' }} mr-2">✓</span> 
                            {{ ucfirst($features['analytics']) }} analytics
                        </li>
                        @if(isset($features['team_members']) && $features['team_members'] > 1)
                        <li class="flex items-start {{ $isPopular ? 'text-gray-600' : 'text-gray-300' }}">
                            <span class="{{ $isPopular ? 'text-green-500' : 'text-green-400' }} mr-2">✓</span> 
                    ...
                    <a href="/register" class="mt-8 block w-full {{ $isPopular ? 'bg-lili-600 hover:bg-lili-700' : ($isFree ? 'bg-gray-700 hover:bg-gray-600' : 'bg-lili-600 hover:bg-lili-700') }} text-white text-center rounded-lg py-3 transition">
                        {{ $isAgency ? 'Contact Sales' : ($isFree ? 'Start for Free' : 'Get Started') }}
                    </a>
                </div>
                @endforeach
            </div>
